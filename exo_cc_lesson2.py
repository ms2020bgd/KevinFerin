...
def get_prices_from_url(url):
    ...
    prices = soup.find_all("span", class_="pricing-table-price")
    li = soup.find_all("ul", class_="pricing-table-list")
    storage = []
    database = []
    for tag in li :
        for lis in tag.findChildren("li") :
            if ('database' in lis.text):
                database.append(int(lis.text.split(' ')[0]))
            if ('storage' in lis.text):
                storage.append(lis.text.split(' ')[0])
    i=0
    for el in soup.find_all("h2") : 
        d[el.text]={
            'price':prices[i].text.replace(' ','').replace('\n','').split('p')[0],
            'storage' : storage[i], 
...
def extract_beer_list_infos(url):
    ...
    result = []
    for item in items :
        result.append(extract_beer_infos("https://www.beerwulf.com/"+item['contentReference']))
    return result
